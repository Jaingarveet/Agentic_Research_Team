...
def create_body_and_sources(state: UserSideInput):
    ...
    intro = state.intro
    convo_hist = state.conversation_history_all_agents
    response = model.with_structured_output(body_and_sources_struct).with_config(temperature = 0.5).invoke(CREATE_BODY_AND_SOURCES_PROMPT.format(convo_hist = convo_hist, intro = intro))
